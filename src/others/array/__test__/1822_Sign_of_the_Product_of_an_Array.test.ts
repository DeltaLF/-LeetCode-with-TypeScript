import { arraySign } from "../1822_Sign_of_the_Product_of_an_Array";

it("test arraySign", () => {
  // one
  expect(arraySign([0])).toBe(0);
  expect(arraySign([10])).toBe(1);
  expect(arraySign([-10])).toBe(-1);
  // two
  expect(arraySign([0, 1])).toBe(0);
  expect(arraySign([-10, -5])).toBe(1);
  expect(arraySign([-1, 5])).toBe(-1);
